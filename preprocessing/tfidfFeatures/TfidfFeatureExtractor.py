...
class TfidfFeatureExtractor(FeatureExtractor):
    ...
    def __init__(self, node_information_df, **kargs):
        ...
        self.index_to_id = dict(zip(range(self.node_information_df.index.size), self.node_information_df.index.values))
        try:
            ...
            print("Doc­doc similarity matrix has been computed, booyah !")
            ...
            for source, target in list_concat:
                cosine_similarities = []
                source_targets = g.neighbors(vertex=self.id_to_index[source], mode='in')
                if source_targets:
